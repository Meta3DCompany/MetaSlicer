...
void PlaterPresetComboBox::update_idex(const Preset& idex_preset)
{
...
Preset find_preset(const deque<Preset>& presets,string preset)
{
    ...
    return presets[0]; //Meta3D: Ugly fix
...
void PlaterPresetComboBox::update()
{
    ...
    std::map<wxString, wxBitmapBundle*>  idex_system_presets;

    //BBS:  move system to the end
    wxString selected_system_preset;
    wxString selected_user_preset;
    wxString tooltip;
    //m_collection->add_pres
    const std::deque<Preset>& presets = m_collection->get_presets();
    
    bool flag = false;
    //m_collection->load_preset();
    //int last_index = 0;
   //bool flag = true;
    //BBS:  move system to the end
    /*if (!presets.front().is_visible)
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        /*
        if(get_preset_name(preset)=="Meta3D XPro")
        {
            i = find_preset(presets,"Meta3D XPro - MIRROR");
            last_index = i;
        }
           
        if(get_preset_name(preset)=="Meta3D XPro - MIRROR")
        {   
            if(flag)
                i = last_index+1;
