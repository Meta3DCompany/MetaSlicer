...
void ObjectList::create_objects_ctrl()
{
    ...
    // Meta3D: Name in the objects side
...
void ObjectList::add_object_to_list(size_t obj_idx, bool call_selection_changed, bool notify_partplate, bool do_info_update)
{
    ...
    //Meta3D: About finding the checkbox
...
void ObjectList::OnEditingStarted(wxDataViewEvent &event)
{
    ...
    auto item = event.GetItem();
    if (col == colPrint) {
        toggle_printable_state(); //Meta3D: Toggle slicing by selecting in the objects tab
        return;
...
//Meta3D: Manipulate this code for closing slicing operation for each object
void ObjectList::toggle_printable_state()
{
    ...
    bool printable = !object(obj_idx)->instances[inst_idx]->printable; //Meta3D: Set print state for object, you can toggle on and off for each object
    ...
    // update printable state on canvas
    //Meta3D: When you close of this script the object won't get shadowed in the prepare tab
    wxGetApp().plater()->get_view3D_canvas3D()->update_instance_printable_state_for_objects(obj_idxs);
